...
export function showBlockedUsers(container, user) {
  ...
  (async function() {
    ...
    blocks.forEach(bu => {
      ...
      row.innerHTML = `
        <span style="font-size:1.05em;flex:1 1 0;">
          ${bu.name || bu.username}
          <span style="color:#999;">@${bu.username}</span>
        </span>
        <button class="blockUnblock"
          style="background:#e74c3c;color:#fff;padding:6px 13px;border:none;border-radius:7px;font-size:1em;">
          Unblock
        </button>
      `;
