export function showUserPanel(container, auth) {
  container.innerHTML = `
    <button id="menuBtn"
      style="position:absolute;top:24px;left:24px;border:none;background:#fff;padding:12px;border-radius:12px;box-shadow:0 2px 8px #ececec;z-index:10;">
      <span style="display:inline-block;font-size:1.55em;">
        &#9776;
      </span>
    </button>
    <div id="drawerOverlay"
      style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.13);z-index:999;">
    </div>
    <nav id="sideDrawer"
      style="position:fixed; top:0; left:0; height:100vh; width:75vw; max-width:400px;
      background:#f8f9fa; box-shadow:2px 0 10px rgba(0,0,0,0.15); z-index:1000;
      transform:translateX(-100%); transition:transform 0.28s cubic-bezier(.77,.2,.56,1.35); display:flex; flex-direction:column;">
  ...
  // Optionally: allow swipe to dismiss
  sideDrawer.addEventListener('touchstart', function(e) {
    this.startX = e.touches[0].clientX;
  });
  sideDrawer.addEventListener('touchmove', function(e) {
    if (this.startX - e.touches.clientX > 80) closeDrawer();
  });

  // Example item logic
  const drawerItems = sideDrawer.querySelectorAll(".drawerItem");
  drawerItems.onclick = () => { alert("Profile clicked!"); closeDrawer(); };
