...
export async function sessionRedirect(auth, container) {
  ...
  if (!user) {
    container.innerHTML = `<div style="color:#c00;background:#ffecec;">Session expired.<br>Please log in again.</div>`;
  ...
  try {
    ...
    if (!resp.ok || data.error) {
      container.innerHTML = `<div style="color:#c00;background:#ffecec;"><b>Session error:</b> ${data.error || "Server error"}</div>`;
      await auth.signOut();
      window.location.hash = "#login";
      return;
    }
    if (user.email.toLowerCase() !== data.email.toLowerCase()) {
      container.innerHTML = `<div style="color:#c00;background:#ffecec;">
