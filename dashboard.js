export function showDashboard(container, user) {
  // ======= DEMO DATA (Replace this with actual API fetch using user token!) =======
  // For API: fetch user balances using user.token, .friendsOwe = [{name, amount}]
  const demo = {
    paidTotal: 342,
    owedTotal: 119,
    youOwe: 41,
    ...
    // Array from backend: each {name, amount} (amount <0 = you owe, >0 = owed to you, 0 = settled)
    friendsOwe: [
  ...
  // WORKING VARS
  const balances = demo.friendsOwe; // Replace with backend
